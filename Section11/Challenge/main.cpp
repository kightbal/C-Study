...
bool find_number(const vector<int> &numbers){
    cout<<"please input number finded!:"<<endl;
    int num_finded {0};
    cin>>num_finded;
    for(auto number:numbers){
        if(number==num_finded)
            return true;
    }
    return false;
}
...
void max_numbers(const vector<int> &numbers){
    if (numbers.size() == 0)
...
void min_numbers(const vector<int> &numbers){
    if (numbers.size() == 0) 
